...
ls_2 = x.get_mod_freqsPerPos_purines_vs_pyrimidines_2(ls_2)
ls_2 = x.normalize_mod_seq_data_purine_pyrimidine_2(ls_2)
...
#top_2 = x.get_mod_freqsPerPos_purines_vs_pyrimidines_2([n.sense_fm_seq for n in temp[0]]) ## hits
#bot_2 = x.get_mod_freqsPerPos_purines_vs_pyrimidines_2([n.sense_fm_seq for n in temp[1]]) ## fails
top_2 = x.get_mod_freqsPerPos_purines_vs_pyrimidines_2([n.antisense_fm_seq for n in temp[0]]) ## hits
bot_2 = x.get_mod_freqsPerPos_purines_vs_pyrimidines_2([n.antisense_fm_seq for n in temp[1]]) ## fails
...
while i<len(ls_2[0]):
	...
	P= [ls_2[0][i],ls_2[1][i],ls_2[2][i],ls_2[3][i],ls_2[4][i],ls_2[5][i]]

	r_top = random_seq_generator.random_mod_gen_pyrim_purine(1,P,40)
	r_bot = random_seq_generator.random_mod_gen_pyrim_purine(1,P,150)
	#r_top = random_seq_generator.random_mod_gen_pyrim_purine(1,P,90)
	#r_bot = random_seq_generator.random_mod_gen_pyrim_purine(1,P,230)
	...
	i+=1
w = csv_io.CSV_Writer('test_stats_mod_and_seq.csv',['pyr_f','pyr_m','pyr_non','pur_f','pur_m','pur_non'],test_stats,True)
w.write_to_file()
cwd = os.getcwd()
